...
def train_and_evaluate_model_kfold(X, Y, trial=None):
    ...
    if trial:
        ...
        # Keep raw copies for metrics later
        Y_split_raw = Y_split.copy()
        y_test_raw  = y_test.copy()

        # Fit scalers
        X_scaler = StandardScaler()
        y_scaler = MinMaxScaler()   # or StandardScaler(), your choice

        X_train = X_scaler.fit_transform(X_split)
        X_test  = X_scaler.transform(X_test)

        y_train = y_scaler.fit_transform(Y_split.reshape(-1, 1))   # <-- use y_scaler
        y_test  = y_scaler.transform(y_test.reshape(-1, 1))        # <-- use y_scaler

        # Tensors (keep shape (B, F) and (B, 1))
        X_train_t = torch.tensor(X_train, dtype=torch.float32)
        y_train_t = torch.tensor(y_train, dtype=torch.float32)
        ...
        for train_index, val_index in kf.split(X_train):
            ...
            for epoch in range(epochs):
                ...
                for xb, yb in train_loader:
                    optimizer.zero_grad()
                    loss = criterion(model(xb), yb)   # both (B,1)
                    loss.backward()
                    optimizer.step()
                ...
                with torch.no_grad():
                    ...
                    y_pred_train_norm = model(X_train_t).cpu().numpy()  # (n,1)
                    y_pred_test_norm  = model(X_test_t ).cpu().numpy()  # (m,1)

                # denormalize with y_scaler (fitted above)
                ...
                # RAW ground truth (we saved these before scaling)
            ...
            # ---- Training subplot ----
            plt.subplot(1, 3, 1)
            sc1 = plt.scatter(true_tr_raw, y_pred_train, c='royalblue', alpha=0.7, edgecolor='k', s=60)
            lims = [min(true_tr_raw.min(), y_pred_train.min()), max(true_tr_raw.max(), y_pred_train.max())]
