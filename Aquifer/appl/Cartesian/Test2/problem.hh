...
namespace Dumux
{
    ...
    public:
        ...
        {   
            ...
            //     // phaseFlux *= volVars.mobility(0);
            //     // phaseFluxn *= volVars.mobility(1);
        ...
        {   
            ...
            for (const auto &element : elements(this->gridGeometry().gridView(), Dune::Partitions::interior))
            {
                ...
                for (auto &&scv : scvs(fvGeometry))
                {
                    ...
                    {
                        if (compIdx < numComponents){
                            MASS_MAT[Grid_ID][compIdx] += volVars.porosity() * scv.volume()* extrusion_ * ((fs.saturation(gasPhaseIdx) * fs.molarDensity(gasPhaseIdx) * fs.moleFraction(gasPhaseIdx, compIdx) + fs.saturation(liquidPhaseIdx) * fs.molarDensity(liquidPhaseIdx) * fs.moleFraction(liquidPhaseIdx, compIdx)));   
                    ...
                    for (int compIdx = 0; compIdx < numComponents; ++compIdx)
                    {
                        ...
                        inventory[compIdx] += volVars.porosity() * scv.volume() * extrusion_  * ((fs.saturation(gasPhaseIdx) * fs.molarDensity(gasPhaseIdx) * fs.moleFraction(gasPhaseIdx, compIdx) + fs.saturation(liquidPhaseIdx) * fs.molarDensity(liquidPhaseIdx) * fs.moleFraction(liquidPhaseIdx, compIdx)));
                        inventoryPastSol[compIdx] += volVarsPast.porosity() * scv.volume() * extrusion_  * ((fsOld.saturation(gasPhaseIdx) * fsOld.molarDensity(gasPhaseIdx) * fsOld.moleFraction(gasPhaseIdx, compIdx) + fsOld.saturation(liquidPhaseIdx) * fsOld.molarDensity(liquidPhaseIdx) * fsOld.moleFraction(liquidPhaseIdx, compIdx)));
                    }
                ...
                {   
                    ...
                    volVars.update(elemSol, *this, element, scv);
                    volVarsPast.update(elemSolPast, *this, element, scv);
                    Scalar extrusion_  = 2 * M_PI * X_min;
                    ...
                    if (scvf.boundary() && scvf.area() == Delta_y/2 && scvf.center()[0] == X_min)
                    {
                        ...
                        if ((globalPos[1] - Y_min >= Y_max - inj_interval) && (Time < localTimeInCycle))
                        {
                            const int cycleNumber = std::floor(Time/(injectionDurationDev+idleDurationDev));
                            const Scalar localTimeInCycle = Time - cycleNumber*(injectionDurationDev+idleDurationDev);
                            if(localTimeInCycle <= injectionDurationDev){
                                ...
                                for (int compIdx = 0; compIdx < numComponents; ++compIdx)
                                {
                                    ...
                                    values_inj[compIdx] += injectionComposition_[compIdx] * scvf.area() * extrusion_ * inj_rate_dev;
                                }
                            }
                        }
                        // if ((globalPos[0] - Delta_x/2 < X_min + eps_) && (globalPos[1] - Y_min >= Y_max - inj_interval) && (Time >= developmentDuration))
                        if ((globalPos[1] - Y_min >= Y_max - inj_interval) && (Time >= developmentDuration))
                        {
                            if(localTimeInCycle < injectionDurationOp){
                                for (int compIdx = 0; compIdx < numComponents; ++compIdx)
                                {
                                    ...
                                    values_inj[compIdx] += injectionComposition_[compIdx] * scvf.area() * extrusion_ * inj_rate_Op;  
                                }
                            }
                            if (localTimeInCycle > injectionDurationOp + idleDurationOp ){
                                for (int compIdx = 0; compIdx < numComponents; ++compIdx)
                                ...
                                for (int compIdx = 0; compIdx < numComponents; ++compIdx)
                                {
                                    ...
                                    values_prod[compIdx] +=  scvf.area() * extrusion_ * ((Extractionrate_)*(X_LIQ*Mobility[liquidPhaseIdx]/(Mobility[liquidPhaseIdx]+Mobility[gasPhaseIdx])) + (Extractionrate_)*(X_GAS*Mobility[gasPhaseIdx]/(Mobility[liquidPhaseIdx]+Mobility[gasPhaseIdx])));
            ...
            // materialBalanceError = inventoryPastSol - inventory + dt * well1.getWellRatePost() + dt * (convertedGeo + convertedMG + convertedSR);
            ...
            std::cout << "The material balance error is: " << error_sum <<  std::endl;
    ...
    private:
        ...
        PrimaryVariables initial_(const GlobalPosition &globalPos) const
        {
            ...
            FluidState fs;
            fs.setMoleFraction(gasPhaseIdx, N2Idx, 1.0); fs.setMoleFraction(gasPhaseIdx, CH4Idx, 0.0); fs.setMoleFraction(liquidPhaseIdx, H2OIdx, 1);
            ...
            // values[contiCH4EqIdx]  =  0.97*(1-xgH2O) * pn / (FluidSystem::fugacityCoefficient(fs,liquidPhaseIdx,CH4Idx) * pw);
            // values[contiH2EqIdx]  =  0.0*(1-xgH2O) * pn / (FluidSystem::fugacityCoefficient(fs,liquidPhaseIdx,H2Idx) * pw);
