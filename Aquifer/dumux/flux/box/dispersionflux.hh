...
public:
    ...
    static constexpr auto referenceSystemFormulation = Dumux::ReferenceSystemFormulation::molarAveraged;
        ...
        {
            const auto rho = massOrMolarDensity(elemVolVars[scv], referenceSystemFormulation, phaseIdx);
        ...
        for (int compIdx = 0; compIdx < numComponents; compIdx++)
        {
            ...
            for (auto&& scv : scvs(fvGeometry))
            {
                const auto x = massOrMoleFraction(elemVolVars[scv], referenceSystemFormulation, phaseIdx, compIdx);
