...
public:
    ...
    struct Params
    {
        Params(Scalar pcEntry, Scalar lambda, Scalar n_w, Scalar n_nw)
        : pcEntry_(pcEntry), lambda_(lambda), n_w_(n_w), n_nw_(n_nw)
        {}
        ...
        Scalar n_w() const { return n_w_; }
        void setN_w(Scalar n_w) { n_w_ = n_w; }

        Scalar n_nw() const { return n_nw_; }
        void setN_nw(Scalar n_nw) { n_nw_ = n_nw; }

        bool operator== (const Params& p) const
        {
            return Dune::FloatCmp::eq(pcEntry(), p.pcEntry(), 1e-6)
                   && Dune::FloatCmp::eq(lambda(), p.lambda(), 1e-6)
                   && Dune::FloatCmp::eq(n_w(), p.n_w(), 1e-6)
                   && Dune::FloatCmp::eq(n_nw(), p.n_nw(), 1e-6);
        }

    private:
        Scalar pcEntry_, lambda_, n_w_, n_nw_;
    };
    ...
    static Params<Scalar> makeParams(const std::string& paramGroup)
    {
        ...
        const auto n_w = getParamFromGroup<Scalar>(paramGroup, "n_w");
        const auto n_nw = getParamFromGroup<Scalar>(paramGroup, "n_nw");
        return {pcEntry, lambda, n_w, n_nw};
    }
    ...
    static Scalar dkrw_dswe(Scalar swe, const Params<Scalar>& params)
    {
        ...
        return params.n_w()*pow(swe, params.n_w() - 1.0);
    ...
    static Scalar krn(Scalar swe, const Params<Scalar>& params)
    {
        ...
        return 0.61 * pow(sne, params.n_nw());
    }
    ...
    static Scalar dkrn_dswe(Scalar swe, const Params<Scalar>& params)
    {
        ...
        return  -0.61 * params.n_nw() * pow((1-swe), params.n_nw() - 1.0);
    }
};
