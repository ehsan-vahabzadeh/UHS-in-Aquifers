...
public:
    ...
    static constexpr int gasCompIdx = CO2Idx; //!< index of the gas component
...
//PENG ROBINSON CUBIC EOS
    ...
    static Scalar zFactor(const FluidState &fluidState)
    {
        ...
        if (Policy::useIdealGasDensity())
            ...
            Scalar sumYiAmix[numComponents], dsmalladxi[numComponents];
            ...
            for (int i=0; i<numComponents;++i){
                ...
                for (int j=0;j<numComponents;++j){
                    ...
                    dsmalladxi[i] += 2*sqrt(aconst[i]) * fluidState.moleFraction(gasPhaseIdx,j)*(1-BIC[i][j])*(sqrt(aconst[j]));
                }
            ...
            Scalar Term1[numComponents], Term3[numComponents], Term5[numComponents];
            Term6 = sqrt(8);
            for (int i=0; i<numComponents; ++i){
                Term1[i]=(bconst[i]/bm)*(temp-1);
                Term2=-log(temp-Bm);
                Term4=-Am/(Bm*Term6);
                Term5[i]=(bconst[i]/bm)-(dsmalladxi[i]/am);
                Term7=2*temp+Bm*(2-Term6);
                Term8=2*temp+Bm*(2+Term6);
                Term3[i]=Term4*(Term5[i])*log(Term7/Term8);
                PHI[i]=exp(Term1[i]+Term2+Term3[i]);
            }
        // return PHI;
            return temp;
            // return 1;
    ...
    static Scalar density(const FluidState &fluidState, int phaseIdx)
    {
        ...
        {
            // assume ideal gas
            std::vector<double> MW(numComponents);
            MW[H2OIdx] = H2O::molarMass();
            MW[CH4Idx] = CH4::molarMass();
            MW[CO2Idx] = CO2::molarMass();
            MW[N2Idx] = N2::molarMass();
            MW[H2Idx] = H2::molarMass();
            ...
            // for (int i=0; i<numComponents; ++i){
            //         // density += fluidState.moleFraction(gasPhaseIdx, i) * MW[i] * molarDensity(fluidState, gasPhaseIdx); 
            //         density += fluidState.moleFraction(gasPhaseIdx, i) * PHI[i] * MW[i] * pressure/(R*temperature);
            // }
            density = IdealGas::density(avgMolarMass, temperature, pressure)/zFactor(fluidState);
            return density;
            // return IdealGas::molarDensity(temperature, pressure) * H2::molarMass();
    ...
    static Scalar molarDensity(const FluidState &fluidState, int phaseIdx)
    {
        ...
        else
        {
            ...
            // return IdealGas::molarDensity(T,p)/zFactor(fluidState);
        }
    }
